    ...
    <MainCard title={title} secondary={<CSVExport {...{ data, headers, filename: 'data.csv' }} />} className="table-card">
      ...
      <Row>
        ...
        <Stack direction="horizontal" className="justify-content-end align-items-center flex-wrap p-4" gap={2}>        
          ...
          <Button variant="success" onClick={() => handleClick("/farms/picketadd")} >
              {'ADD PICkET'}
          </Button>
...
export default function BasicTable({ title, striped, footer = false }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetcher(['pickets?current_page=1&regs_per_page=10000', {}]);   
...
const actionIcons = [
      ...
      swalWithBootstrapButtons
        ...
        .then(async(result) => {
          if (result.isConfirmed) {
            try {
                const response = await fetcherDelete(['pickets/' + id, {}]);
                // Refresh the data after deletion
                const updatedData = await fetcher(['pickets?current_page=1&regs_per_page=10000', {}]);
  ...
  const handleEdit = (id) => {
    ...
    window.location.href = `/farms/picketadd/${id}`;
  };  


  const columns = useMemo(
    () => [     
      { header: 'Picket Description', accessorKey: 'picket_description', 
        cell: (cell) => (
          <Stack >
            <strong className='text-nowrap'>{cell.getValue()}</strong>
            <span className="text-muted small"><i className='ph ph-barn'></i> {cell.row.original.farm_name}</span>
          </Stack>
        )    
      },
      { header: 'area', accessorKey: 'area', meta: {className: 'text-end'} },
      
      {
        header: 'Profile Progress',
        accessorKey: 'progress',
        cell: ({getValue}) => {          
          const class_name = getValue() == 100 ? 'progress-success' : getValue() < 68 ? 'progress-warning': 'progress-primary' ;
