    ...
    <MainCard title={title} secondary={<CSVExport {...{ data, headers, filename: 'data.csv' }} />} className="table-card">
      ...
      <Row>
        ...
        <Stack direction="horizontal" className="justify-content-end align-items-center flex-wrap p-4" gap={2}>        
          ...
          <Button variant="success" onClick={() => handleClick("/animals/animaladd")} >
                {'ADD ANIMAL'}
          </Button>
...
export default function BasicTable({ title, striped, footer = false }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetcher(['animals?current_page=1&regs_per_page=1000', {}]);     
...
const actionIcons = [ 
      ...
      swalWithBootstrapButtons
        ...
        .then(async(result) => {
          if (result.isConfirmed) {
            try {
                const response = await fetcherDelete(['animals/' + id, {}]);
                // Refresh the data after deletion
                const updatedData = await fetcher(['animals?current_page=1&regs_per_page=5', {}]);
  ...
  const handleEdit = (id) => {
    ...
    window.location.href = `/animals/animaladd/${id}`;
  };  
  ...
  const columns = useMemo(
    () => [
      { header: 'Lot Description', meta: { className: 'text-nowrap' }, 
        accessorKey: 'lot_description',
        cell: (cell) => {
        return (
          <Stack >
            <strong>{cell.getValue()}</strong>
            <span className="text-muted small"><i className='ph ph-map-pin-simple-area'></i>{ ` ${cell.row.original.picket_description} | ${cell.row.original.farm_name}`}</span>  
          </Stack>
        )}
      },
      { header: 'Earring Number', accessorKey: 'earring_number', meta: { className: 'text-end'} },
      { header: 'Breed', accessorKey: 'breed'},
      { header: 'Sex', accessorKey: 'sex'},
      { header: 'Age', accessorKey: 'age' },     
      { header: 'Entry Weight', accessorKey: 'entry_weight', meta: { className: 'text-end'} },
      { header: 'SISBOV BRA', accessorKey: 'sisbov_mapa_br' }, 
      {
        //comment('A - Ativo, V - Vendido, M - Morto, R - Retirado');
        id: 'status',
        header: 'Status',
        accessorKey: 'status',
        cell: (cell) => {
          ...
          switch (upperCase(cell.getValue())) {
            case 'V':
              status_description = 'Sold'
              bg = 'light-warning';
              break;
            case 'M':
              status_description = 'Dead'
              bg = 'light-warning';
              break;
            case 'R':
              status_description = 'Removed'
              bg = 'light-info';
              break;
            case 'A':
            default:
          ...
          return <Badge bg={bg}>{status_description}</Badge>;
        }           
      },
      {
